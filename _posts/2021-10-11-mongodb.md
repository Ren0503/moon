...
Model là các đối tượng sẽ được Mongoose ánh xạ thành các document trong MongoDB. Với model ta có thể định nghĩa cấu trúc document trong MongoDB, thiết lập dữ liệu mặc định, các phương thức xác thực, ... Mongoose cũng sẽ cung cấp cho model những interface cho tạo, truy vấn, cập nhật hay xoá dữ liệu. Để tạo Model trước tiên ta cần tạo schema, sau đó thông qua schema mà định nghĩa model.
...
7. **ObjectId** : lưu trữ `_id` của model khác, dùng cho liên kết hai document với nhau. Tương tự **FOREIGNKEY** trong SQL.
